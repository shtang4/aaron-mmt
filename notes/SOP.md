...
1. TradingView: nearest demand zone below (long) or supply zone above (short), and any FVG. Take the **near edge** — that is the first price the retrace meets and where it gets **swept** before continuing, which is the whole reason the entry sits there. Colours: **supply pink · demand blue · bullish FVG light green · bearish FVG dark red**.
2. Bybit: **EMA 9 on the retrace side** — **below** price for a long, **above** price for a short. If the zone edge sits deeper, the EMA closest to the edge. That is the price. (If the line you want is on the wrong side of price, the pullback has not started; there is nothing to rest an order at yet.) **Any of the five lines is parkable** — EMA 9 fills most often, EMA 155 is the deep one with the pivot behind it, EMA 255 is "the last guard". Trail the order up the line as the trend advances.
...
1. Isolated margin. Leverage **1× to 5× as a beginner**, 1× to 10× once past beginner (session). Capital you can lose — "not using for essential purpose". **Low leverage is not safety:** liquidation sits about `100 ÷ leverage` percent away (5× → 20%, 25× → 4%). The **stop** caps the loss; the leverage only sets how much room it has.
   **The leverage test that settles it — his own number: "a 3% dip is a normal pullback."** At 5× that is −15% ROI (inside the beginner stop tier). At 20× it is **−60%**, past the intermediate tier. At 50× it is **−150% — a liquidation**. Pick a leverage where a routine pullback does not reach your stop.
2. **SSWB is a ladder of profit targets, not a stake** (session): **two figures** ($10–99 per trade) → **three figures** ($100–999) → **four figures**. Move up a rung only when the current one is routine — "don't be greedy, start with two figure ... train your mindset, then slowly scale up".
3. Margin per trade: **$20 to learn — the one stake figure the cards actually confirm** (four of six positions on his own "10 trades" screen imply $19.47–$20.05); then 1% of wallet; never above 10%. Bigger wallet, smaller trade. **Size by percentage, never by a fixed figure** — the deck's "$150 per trade" does not survive the one slide where it is checkable (the RAVE cards imply $324 and $224), and he says himself "I didn't even start with hundred fifty, I scale up accordingly". **Loss at the stop never above 1–2% of account equity**, whatever the confluence score.
   **On the testimonial leverage.** The results cards run at 15×, 20×, 25×, 50× and — on the student case — **200×**, where liquidation sits about **0.5%** of adverse price movement away. His own current practice is "never aim the maximum leverage". Read the cards as evidence of leverage, not of method.
4. **Check the heatmap before you place the stop.** A cluster just beyond it will be swept before the move you want — move the stop past it or lower the leverage. A cluster is a **magnet, never support**: price is drawn into it and the forced orders accelerate through it.
...
5. Stop as ROI on margin by tier: beginner 5–20%, intermediate 20–50%, advanced 50%+. Convert to price; it must sit beyond the structural level (far edge of the zone, or the heel low). If it does not, **lower the leverage** until it does (session-confirmed). Never move the level, never widen the tier.
6. **Risk first, margin second.** Ask "how much can I afford to lose if I'm wrong" *before* choosing a margin. `margin × leverage = position size`; `P&L = margin × ROI`. You control margin and leverage completely and the price move not at all — a bigger margin multiplies whatever ROI arrives, **including a negative one**.
7. **Late entry → smaller dose.** Early, with time to manage it → scale bigger, take partials, trail. Late, the move already running → **small size, quick trade, in and out**. Lateness is a sizing input, not only a warning.
8. **Never copy anyone's position size.** A trade call gives you a price, never a size. Size comes from your wallet and your tolerance — it is the one number no call can supply.
9. Write the money at risk in USDT before entry. **Size on three inputs — capital, experience and emotional tolerance — never on confidence.** "Do you decide your position size based on how confident you feel?" If you could not sit through that stop calmly, the size is wrong, and that is the whole fix for exiting winners early.
...
3. Climax exit — **MAVol → MACD → Take profit**, which is what M-M-T stands for in this rule. A volume bar **above both VOLMA lines** ("abnormal", session-defined — no multiple) + the MACD histogram at its **extreme bar**: the **mountain peak** if you are long, the **iceberg dip** if you are short, confirmed when the next bar prints shorter — **on Bybit that next bar is drawn hollow** → close 75%. Move the runner's SL to entry. *(The slide says PTP 75%; the session says 25%. Near-opposite plans — 75% kept here as the written figure, flagged in [04 §2.1](04-sniper-entry-exit.md).)*
...
5. Runner: **prefer the Hunting SL to a breakeven stop.** A stop moved to entry returns **0%** on a fast reversal — "suddenly one long wick, you gave it back everything" — while a Hunting SL locked at 20% returns 20%. A breakeven stop protects the capital; only a hunting stop protects the profit. It locks a rising ROI. EMA 9 crossing back through EMA 25 is the backstop **for the runner only** — on a full position the fast lines crossing each other is noise while all three sit above EMA 155.
...
4. **The one hedging case:** a position opened in cross mode by mistake. **He scopes it the same way** — "you protected for your isolated mode is fine, then you don't even need to use hedging." Hedge to freeze exposure, then close it or switch it to isolated, then close the hedge. Damage control for an execution error, nothing more.
   **If you do it, match the notional: same margin, same leverage, opposite side.** `notional = margin × leverage`, so equal products are flat. His margin rule (losing >50% → hedge with 100% of the original margin) is exactly right; his "maximise the leverage on the hedge leg" instruction is what turns it into a leveraged reversal — $100 at 15× hedged by $100 at 25× leaves you **net short $1,000**, not flat.
   **His trigger, and it is strict — use it:** price closed **below EMA 155** (better, below 255) **and CHoCH followed by BOS** (a CHoCH alone is a retracement — "you let it be") **and the Donchian ribbon has flipped**. Monitor on the higher frames, execute on 1-minute, enter on the retrace. Almost no losing trade qualifies. Note the same conditions describe a capitulation low, which is where the course elsewhere *buys*.
